...
LogRotator.stream = function (opts) {
	...
	if (this.dirExists()) {
		...
		throw new Error('Target log directory does not exist! We\'re not going to create the directory for you.');
	}
};
...
LogRotator.getFileName = function () {
	...
	if (this.file_name) {
		tmpStr += this.file_name;
	}

	return tmpStr + this.last_date_str + getFileExtension(this.file_extension);
};
